...
app_section = st.sidebar.radio("Go to Section", ["📊 Analysis", "🔮 Prediction"])

if app_section == '📊 Analysis':
    import streamlit as st
    import numpy as np
    import pandas as pd
...
# ==============================
if app_section == "🔮 Prediction":
    st.header("🔮 Real-Time Diabetes Prediction on All Features")
    ...
    if 'target' in data.columns:
        ...
        feature_names_all = features_only.columns.tolist()

        # Prepare data
        X_full = features_only.values
        y_full = data['target'].values

        # Scale features
        full_scaler = StandardScaler()
        X_scaled_full = full_scaler.fit_transform(X_full)

        # Train both models
        model_firefly = LogisticRegression(max_iter=2000)
        model_firefly.fit(X_scaled_full, y_full)

        model_corr = LogisticRegression(max_iter=2000)
        model_corr.fit(X_scaled_full, y_full)

        st.markdown("### Input patient data below to get predictions from both models")
        user_input_values = {}
        for feat in feature_names_all:
            user_input_values[feat] = st.number_input(f"Enter `{feat}`", value=0.0, format="%.4f")

        input_vector = np.array([user_input_values[feat] for feat in feature_names_all]).reshape(1, -1)
        input_scaled = full_scaler.transform(input_vector)

        tab1, tab2 = st.tabs(["🔥 Firefly Logistic Regression", "📎 Correlation-based Logistic Regression"])

        with tab1:
            pred_firefly = model_firefly.predict(input_scaled)[0]
            prob_firefly = model_firefly.predict_proba(input_scaled)[0][1]
            st.subheader("🔥 Firefly Logistic Regression Result")
            st.success(f"Prediction: {'Positive (1)' if pred_firefly == 1 else 'Negative (0)'}")
            st.info(f"Probability of having diabetes: **{prob_firefly:.2%}**")

        with tab2:
            pred_corr = model_corr.predict(input_scaled)[0]
            prob_corr = model_corr.predict_proba(input_scaled)[0][1]
            st.subheader("📎 Correlation-based Logistic Regression Result")
            st.success(f"Prediction: {'Positive (1)' if pred_corr == 1 else 'Negative (0)'}")
            st.info(f"Probability of having diabetes: **{prob_corr:.2%}**")
    else:
        st.warning("The dataset must contain a 'target' column for prediction.")
